    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.roles.update", [$role->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $role->price) }}" step="0.01" required>
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('features') ? 'is-invalid' : '' }}" name="features" id="features">{!! old('features', $role->features) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('validity') ? 'is-invalid' : '' }}" name="validity" id="validity" required>
                    ...
                    @foreach(App\Models\Role::VALIDITY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('validity', $role->validity) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('products_limit') ? 'is-invalid' : '' }}" type="number" name="products_limit" id="products_limit" value="{{ old('products_limit', $role->products_limit) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('shop_limit') ? 'is-invalid' : '' }}" type="number" name="shop_limit" id="shop_limit" value="{{ old('shop_limit', $role->shop_limit) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('customers_limit') ? 'is-invalid' : '' }}" type="number" name="customers_limit" id="customers_limit" value="{{ old('customers_limit', $role->customers_limit) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('orders_limit') ? 'is-invalid' : '' }}" type="number" name="orders_limit" id="orders_limit" value="{{ old('orders_limit', $role->orders_limit) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('stock_management') ? 'is-invalid' : '' }}" name="stock_management" id="stock_management" required>
                    ...
                    @foreach(App\Models\Role::STOCK_MANAGEMENT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('stock_management', $role->stock_management) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('new_courier_add') ? 'is-invalid' : '' }}" name="new_courier_add" id="new_courier_add" required>
                    ...
                    @foreach(App\Models\Role::NEW_COURIER_ADD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('new_courier_add', $role->new_courier_add) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>

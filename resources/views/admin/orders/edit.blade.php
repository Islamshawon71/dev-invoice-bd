                ...
                <div class="card-body">
                            ...
                            <div class="form-group {{ $errors->has('shop') ? 'has-error' : '' }}">
                                ...
                                <select class="form-control select2" name="shop_id" id="shop_id">
                                    @foreach($shops as $id => $entry)
                                        <option value="{{ $id }}" {{ (old('shop_id') ? old('shop_id') : $order->shop_id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('shop'))
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input class="form-control {{ $errors->has('customer_name') ? 'is-invalid' : '' }}"
                                    ...
                                    value="{{ old('customer_name', $order->customer->customer_name ) }}" required>
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                ...
                                <input class="form-control {{ $errors->has('customer_phone') ? 'is-invalid' : '' }}"
                                    ...
                                    value="{{ old('customer_phone', $order->customer->customer_phone ) }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control {{ $errors->has('customer_address') ? 'is-invalid' : '' }}"
                            name="customer_address" id="customer_address">{{ old('customer_address', $order->customer->customer_address) }}</textarea>
                    ...
                    <div class="form-group {{ $errors->has('courier') ? 'has-error' : '' }}">
                        ...
                        <select class="form-control select2" name="courier_id" id="courier_id">
                            ...
                            <option value="{{ $id }}" {{ (old('courier_id') ? old('courier_id') : $order->courier_id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group {{ $errors->has('courier_tracking_number') ? 'has-error' : '' }}">
                        ...
                        <input class="form-control" type="text" name="courier_tracking_number" id="courier_tracking_number" value="{{ old('courier_tracking_number', $order->courier_tracking_number) }}">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($order->products as $product)
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="hidden" name="product['+e.params.data.id+'][product_code]" value="' + e.params.data.product_code + '"><span class="product_code">{{ $product->product->product_code }}</span></td>
                                            <td><input type="hidden" name="product['+e.params.data.id+'][product_name]" value="' + e.params.data.text + '"><span class="product_name">{{ $product->product->product_name }}</span></td>
                                            <td><input type="hidden" name="product['+e.params.data.id+'][price]" value="' + e.params.data.product_price + '"><span class="product_price">{{ $product->price }}</span></td>
                                            <td><input type="number" name="product['+e.params.data.id+'][quantity]" class="product_quantity form-control" style="width:80px;" value="{{ $product->quantity }}"></td>

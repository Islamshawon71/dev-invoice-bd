...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tickets.update", [$ticket->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="description">{{ trans('cruds.ticket.fields.description') }}</label>
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $ticket->description) }}</textarea>
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.ticket.fields.status') }}</label>
...
@if(isset($ticket) && $ticket->file)
      var files = {!! json_encode($ticket->file) !!}
          for (var i in files) {
          var file = files[i]
